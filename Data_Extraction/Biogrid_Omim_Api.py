    ...
    try:
        ...
        for bid, omim in omim_link.items():
            if omim != "0":
                ...
                for variant in allelicVariant:
                    base_entry = {
                        'Biogrid_id': bid,
                        'preferredTitle': variant['allelicVariant']['preferredTitle'],
                        'mimNumber': variant['allelicVariant']['mimNumber']
                    }
                    
                    if 'name' in variant['allelicVariant']:
                        base_entry['Phenotype'] = variant['allelicVariant']['name']
                    if 'mutations' in variant['allelicVariant']:
                        base_entry['mutations'] = variant['allelicVariant']['mutations']
                    
                    if 'dbSnps' in variant['allelicVariant']:
                        dbsnps = variant['allelicVariant']['dbSnps']
                        if ',' in dbsnps:
                            # Split dbSnps and create individual entries
                            for dbsnp in dbsnps.split(','):
                                entry = base_entry.copy()
                                entry['dbSnps'] = dbsnp.strip()
                                omim_out.append(entry)
                        else:
                            # Single dbSnp
                            base_entry['dbSnps'] = dbsnps
                            omim_out.append(base_entry)
                    else:
                        # No dbSnps field
                        omim_out.append(base_entry)
